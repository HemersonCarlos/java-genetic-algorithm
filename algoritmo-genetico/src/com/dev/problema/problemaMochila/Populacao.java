...
public class Populacao {
	...
	public Individuo getPrimeiroIndividuoComMaiorPontuacao() {
		...
		for(int contador = 0; contador < individuos.length; contador++) {	
			if(individuos[contador].getFitnessPorIndividuo() > valorAtualComparado && individuos[contador].getPesoDaMochila() <= Regra.PESO_MAXIMO_DA_MOCHILA) {
	...
	public Individuo getSegundoIndividuoComMaiorPontuacao() {
		...
		for(int contador = 0; contador < individuos.length; contador++) {
			if(individuos[contador].getFitnessPorIndividuo() > valorAtualComparado && individuos[contador].getPesoDaMochila() <= Regra.PESO_MAXIMO_DA_MOCHILA && individuos[contador].getFitnessPorIndividuo() != this.getValorDeFitnessDoMelhorIndividuo()) {
				valorAtualComparado = individuos[contador].getFitnessPorIndividuo();
	...
	public int getIndiceDoSegundoPiorIndividuoComMenorPontuacao() {
		...
		for(int contador = 0; contador < individuos.length; contador++) {
			if(individuos[contador].getFitnessPorIndividuo() < valorAtualComparado && individuos[contador].getPesoDaMochila() <= Regra.PESO_MAXIMO_DA_MOCHILA && (individuos[contador].getFitnessPorIndividuo() == this.getPrimeiraMenorPontuacao() && contador != this.getIndiceDoPrimeiroPiorIndividuoComMenorPontuacao())) {
				valorAtualComparado = individuos[contador].getFitnessPorIndividuo();
				indiceDoIndividuoComMenorValorDeFitness = contador;
			}
		}
